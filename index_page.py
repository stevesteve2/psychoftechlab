...
// ===== method icon: big data rain =====
(function(){
  const cv=document.getElementById('mData'); if(!cv) return;
  const ctx=cv.getContext('2d'), W=cv.width, H=cv.height, cols=10;
  const drops=Array.from({length:cols},()=>Math.random()*H);
  function draw(){
    ctx.fillStyle=css('--surface')||'#fff'; ctx.globalAlpha=0.28; ctx.fillRect(0,0,W,H); ctx.globalAlpha=1;
    ctx.font='13px ui-monospace,monospace';
    const accent=css('--accent'), mutedC=css('--muted');
    drops.forEach((y,i)=>{
      const x=8+i*(W-16)/cols;
      ctx.fillStyle=Math.random()<0.2?accent:mutedC;
      ctx.fillText(Math.random()<0.5?'0':'1', x, y);
      drops[i]=y>H+10?0:y+3+Math.random()*3;
    });
    if(!reduceMotion) requestAnimationFrame(draw);
  }
...
(function(){
  ...
  function draw(){
    ...
    ctx.font='10px Archivo,sans-serif'; ctx.fillStyle=mutedC;
    ctx.fillText('treatment',18,H-44); ctx.fillText('control',W/2+20,H-44);
    // coin
...
// ===== method icon: rotating globe =====
(function(){
  const cv=document.getElementById('mGlobe'); if(!cv) return;
  const ctx=cv.getContext('2d'), W=cv.width, H=cv.height, R=48, cx=W/2, cy=H/2;
  let ph=0;
  function draw(){
    ctx.clearRect(0,0,W,H);
    const accent=css('--accent'), mutedC=css('--muted');
    ctx.strokeStyle=mutedC; ctx.lineWidth=1.4;
    ctx.beginPath(); ctx.arc(cx,cy,R,0,6.29); ctx.stroke();
    // latitudes
    [-0.55,0,0.55].forEach(f=>{
      ctx.beginPath(); ctx.ellipse(cx,cy+f*R,Math.sqrt(1-f*f)*R,Math.sqrt(1-f*f)*R*0.28,0,0,6.29);
    ...
    for(let k=0;k<4;k++){
      ...
      ctx.globalAlpha=0.5; ctx.strokeStyle=mutedC; ctx.stroke(); ctx.globalAlpha=1;
    }
    // study sites blinking
    const sites=[[0.3,-0.4],[-0.5,0.1],[0.1,0.45],[-0.2,-0.5],[0.55,0.2]];
    sites.forEach(([sx,sy],i)=>{
      const lon=Math.cos(ph*1.2+i*1.3);
      if(lon>0){ ctx.beginPath();
        ctx.arc(cx+sx*R*lon, cy+sy*R, 3, 0, 6.29);
        ctx.fillStyle=accent; ctx.globalAlpha=0.5+0.5*Math.sin(ph*3+i); ctx.fill(); ctx.globalAlpha=1; }
    });
    ph+=0.012;
    if(!reduceMotion) requestAnimationFrame(draw);
  }
